...
DO UPDATE SET
  ...
  updated_at = NOW();
`;
export async function runBillingUsageDailyRollup(db, log) {
    const now = new Date();
...
export async function runBillingCouponExpiryCleanup(db, log) {
    const result = await db.query(`UPDATE coupons
...
export async function runBillingTrialExpiryCheck(db, log) {
    const result = await db.query(`SELECT COUNT(*)::text AS count
       FROM organization_subscriptions
      WHERE status = 'trialing'
        AND trial_end IS NOT NULL
        AND trial_end < NOW()`);
    log.info({ expiredTrials: Number(result.rows[0]?.count ?? 0) }, 'billing trial expiry scan complete; provider downgrade flow still needs service integration');
}
export async function runBillingUsageLimitWarningSweep(db, log) {
    const result = await db.query(`WITH current_usage AS (
       SELECT
         s.org_id,
...
export async function runBillingSubscriptionRenewalReconciliation(db, log) {
    const result = await db.query(`SELECT COUNT(*)::text AS count
       FROM organization_subscriptions
      WHERE status IN ('active', 'trialing', 'past_due')
        AND current_period_end < NOW()`);
    log.info({ subscriptionsPastPeriodEnd: Number(result.rows[0]?.count ?? 0) }, 'billing renewal reconciliation scan complete; provider polling not implemented yet');
}
export async function runBillingDunningRetry(db, log) {
    const result = await db.query(`SELECT COUNT(*)::text AS count
       FROM organization_subscriptions
      WHERE status = 'past_due'`);
    log.info({ pastDueSubscriptions: Number(result.rows[0]?.count ?? 0) }, 'billing dunning retry scan complete; retry orchestration not implemented yet');
}
export async function runBillingPlanLimitEnforcementSweep(db, log) {
    const result = await db.query(`WITH current_usage AS (
       SELECT
         s.org_id,
         p.event_limit_monthly,
    ...
    log.info({ hardCapOrgsExceeded: Number(result.rows[0]?.count ?? 0) }, 'billing plan limit enforcement sweep complete');
}
