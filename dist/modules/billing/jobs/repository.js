...
export class BillingJobsRepository {
    ...
    async renewSubscriptions(batchSize) {
    ...
    async generateInvoices(batchSize) {
        return this.withTransaction(async (client) => {
            const result = await client.query(`
        WITH candidates AS (
    ...
    async reconcilePayments(batchSize) {
        return this.withTransaction(async (client) => {
    ...
    async retryWebhooks(batchSize, maxRetries) {
        return this.withTransaction(async (client) => {
            const result = await client.query(`
        WITH due AS (
          SELECT id
    ...
    async deadLetterWebhooks(batchSize, maxRetries) {
        return this.withTransaction(async (client) => {
            const result = await client.query(`
        WITH due AS (
    ...
    async aggregateUsage(batchSize) {
        return this.withTransaction(async (client) => {
            const result = await client.query(`
        WITH due AS (
    ...
    async resetAiCredits(batchSize) {
        return this.withTransaction(async (client) => {
            const result = await client.query(`
        WITH due AS (
    ...
    async expireFeatureOverrides(batchSize) {
        return this.withTransaction(async (client) => {
            const result = await client.query(`
        WITH due AS (
          SELECT id, organization_id
    ...
    async markInvoiceReminders(batchSize, days) {
        return this.withTransaction(async (client) => {
            const result = await client.query(`
        WITH due AS (
    ...
    async cleanupPartitions(retentionDays) {
    ...
    async detectUsageAnomalies(batchSize, multiplier, minimumEvents) {
        return this.withTransaction(async (client) => {
            const result = await client.query(`
        WITH recent AS (
    ...
    async refreshEntitlements(batchSize) {
        return this.withTransaction(async (client) => {
            const result = await client.query(`
        WITH due AS (
    ...
    async archiveAuditLogs(retentionDays) {
        return this.cleanupPartitions(retentionDays);
    }
    async reconcileData(batchSize) {
        return this.withTransaction(async (client) => {
            const result = await client.query(`
        WITH aggregates AS (
          SELECT organization_id,
    ...
    async updateWithAudit(sql, batchSize) {
        return this.withTransaction(async (client) => {
