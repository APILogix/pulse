...
import { authorizationCodeGrant, buildAuthorizationUrl, calculatePKCECodeChallenge, ClientSecretPost, discovery, randomNonce, randomPKCECodeVerifier, randomState, } from 'openid-client';
import { env as config } from '../../../../config/env.js';
import { decrypt } from '../../../../shared/utils/encryption.js';
import { logAudit } from '../../../../shared/middleware/audit-logger.js';
import { oidcLoginStateCache } from '../../infrastructure/cache/auth.cache.js';
...
import { getApiOidcCallbackUrl } from '../../infrastructure/config/oauth-callback.config.js';
...
async function buildOidcConfig(provider) {
    const clientSecret = decrypt(provider.oidc_client_secret_encrypted, config.ENCRYPTION_KEY);
    return discovery(new URL(provider.oidc_issuer), provider.oidc_client_id, clientSecret, ClientSecretPost(clientSecret));
}
/**
 * Unified SSO login entry: routes to OIDC or SAML based on provider_type.
 */
export async function startSsoLogin(input, ipAddress, userAgent, requestId) {
    if (input.provider_id) {
        const ref = await repository.findSsoProviderRef(input.provider_id);
        if (!ref) {
            throw new AuthError('SSO provider not found', AuthErrorCodes.SSO_NOT_CONFIGURED, 404);
        }
...
async function startOidcLogin(input, ipAddress, userAgent, requestId) {
    let provider = null;
    if (input.provider_id) {
    ...
    if (!provider) {
        throw new AuthError('No OIDC provider configured for this request', AuthErrorCodes.OIDC_NOT_CONFIGURED, 404);
    }
...
export async function completeSsoCallback(callbackUrl, ipAddress, userAgent, requestId) {
    ...
    if (!flow) {
        throw new AuthError('SSO session expired; restart sign-in', AuthErrorCodes.OIDC_CALLBACK_INVALID, 400);
    }
    oidcLoginStateCache.delete(state);
    const provider = await repository.findOidcProviderById(flow.providerId);
    if (!provider) {
        throw new AuthError('SSO provider not found', AuthErrorCodes.OIDC_NOT_CONFIGURED, 404);
    }
    const oidcConfig = await buildOidcConfig(provider);
    ...
    if (!email) {
        throw new AuthError('Identity provider did not return an email', AuthErrorCodes.OIDC_CALLBACK_INVALID, 400);
    }
    const displayName = typeof claims?.name === 'string'
        ...
        : typeof claims?.given_name === 'string'
            ? claims.given_name
            : undefined;
    const user = await resolveSsoUser(email, displayName, provider, ipAddress, requestId, 'user.sso_jit_provisioned');
