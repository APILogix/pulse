...
export function extractEmailFromSamlProfile(profile) {
    const candidates = [
        profile.email,
        profile.mail,
        profile.nameID,
...
export function extractDisplayNameFromSamlProfile(profile, email) {
    const candidates = [
        profile.displayName,
        profile['http://schemas.xmlsoap.org/ws/2005/05/identity/claims/name'],
        profile.firstName,
 ...
 */
export async function resolveSsoUser(email, displayName, provider, ipAddress, requestId, auditAction) {
    ...
    if (!provider.oidc_jit_provision) {
        throw new AuthError('No account exists for this identity. Contact your administrator.', AuthErrorCodes.JIT_PROVISIONING_DISABLED, 403);
    }
    const emailDomain = normalized.split('@')[1]?.toLowerCase();
    if (provider.domain &&
        emailDomain &&
        provider.domain.trim().toLowerCase() !== emailDomain) {
        throw new AuthError('Email domain does not match organization SSO policy', AuthErrorCodes.SSO_DOMAIN_MISMATCH, 403);
    }
    ...
    const user = await repository.withTransaction(async (client) => {
        const created = await repository.createSsoJitUser({ id: randomUUID(), email: normalized, full_name: fullName }, client);
        await repository.addOrgMemberSsoProvision(provider.org_id, created.id, role, client);
        return created;
    });
    logAudit({
        user_id: user.id,
...
//# sourceMappingURL=sso-provision.service.js.map