...
export const GENERIC_PASSWORD_RESET_MESSAGE = 'If the email exists, a password reset link has been sent';
export const GENERIC_VERIFICATION_MESSAGE = 'If the account exists and is not verified, a verification email has been sent';
export const GENERIC_REGISTRATION_MESSAGE = 'Account creation request received. Check your email to continue.';
export const randomBytesAsync = promisify(randomBytes);
export function looksLikeRawUserAgent(value) {
    if (!value)
        return false;
...
export function verifyBackupCodeHash(plain, hashed) {
...
export function assertUserUsable(user) {
    if (user.deleted_at) {
...
export function getUserPasswordHashes(user) {
...
export async function ensurePasswordNotReused(user, newPassword) {
 ...
 * cutoff are rejected.
 */
export function markAllUserTokensRevoked(userId) {
    cacheRevokeAllUserTokens(userId);
}
/** Revoke every active session for a user AND blacklist every in-flight token. */
export async function revokeAllSessionsAndTokens(userId, reason) {
 ...
 */
export async function blacklistOtherUserSessionTokens(userId, currentSessionId) {
...
export function maskEmailForHint(email) {
 ...
 */
export function buildMfaDisplayHint(deviceType, deviceName, opts = {}) {
    switch (deviceType) {
        case 'email':
...
export function buildTotp(secretBase32, label) {
...
export function verifyTotpDeviceCode(device, code) {
