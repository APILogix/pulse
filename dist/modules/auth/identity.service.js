...
import { accountDeletionConfirmTemplate, accountUnlockTemplate, emailChangeConfirmTemplate, } from '../../shared/email/templates.js';
...
import { isProviderConfigured, listConfiguredLinkProviders, } from './identity-link.config.js';
import { getPasswordPolicy } from './policy.service.js';
import * as repository from './repository.js';
import { AuthError, AuthErrorCodes, } from './types.js';
import { ACCOUNT_DELETION_GRACE_SECONDS, ACCOUNT_DELETION_TOKEN_TTL_SECONDS, ACCOUNT_UNLOCK_TTL_SECONDS, EMAIL_CHANGE_TTL_SECONDS, generateEmailFlowToken, hashEmailFlowToken, normalizeEmail, } from './utils.js';
...
export async function requestEmailChange(userId, input, ipAddress, requestId) {
    ...
    if (newEmail === normalizeEmail(user.email)) {
        throw new AuthError('New email must differ from current email', AuthErrorCodes.VALIDATION_ERROR, 400);
    }
    const collision = await repository.findUserByEmailHash(emailToHash(newEmail));
    if (collision && collision.id !== userId) {
        throw new AuthError('Email address is not available', AuthErrorCodes.EMAIL_IN_USE, 409);
    }
    ...
    return {
        message: 'Confirmation email sent to the new address. Your current email remains active until you confirm.',
    };
}
export async function confirmEmailChange(input, ipAddress, requestId) {
    const tokenHash = hashEmailFlowToken('email_change', input.token);
    let userId = null;
    await repository.withTransaction(async (client) => {
        const consumed = await repository.consumeEmailVerificationToken(tokenHash, 'email_change', client);
        if (!consumed) {
            throw new AuthError('Invalid or expired email change token', AuthErrorCodes.EMAIL_VERIFICATION_INVALID, 400);
        }
        const collision = await repository.findUserByEmailHash(emailToHash(consumed.email), client);
        if (collision && collision.id !== consumed.user_id) {
            throw new AuthError('Email address is no longer available', AuthErrorCodes.EMAIL_IN_USE, 409);
        }
        const updated = await repository.updateUserEmail(consumed.user_id, consumed.email, emailToHash(consumed.email), client);
        if (!updated) {
            throw new AuthError('User not found', AuthErrorCodes.USER_NOT_FOUND, 404);
        }
        userId = updated.id;
...
export async function requestAccountUnlock(input, ipAddress, requestId) {
...
export async function confirmAccountUnlock(input, ipAddress, requestId) {
    ...
    let userId = null;
    await repository.withTransaction(async (client) => {
        const consumed = await repository.consumeEmailVerificationToken(tokenHash, 'account_unlock', client);
        if (!consumed) {
            throw new AuthError('Invalid or expired unlock token', AuthErrorCodes.EMAIL_VERIFICATION_INVALID, 400);
        }
...
export async function requestAccountDeletion(userId, input, ipAddress, requestId) {
    const user = await repository.findUserById(userId);
    if (!user) {
        throw new AuthError('User not found', AuthErrorCodes.USER_NOT_FOUND, 404);
    }
    if (user.deletion_scheduled_at) {
        throw new AuthError('Account deletion is already scheduled', AuthErrorCodes.DELETION_ALREADY_SCHEDULED, 409, { scheduled_at: user.deletion_scheduled_at.toISOString() });
    }
    const scheduledAt = new Date(Date.now() + ACCOUNT_DELETION_GRACE_SECONDS * 1000);
    const token = generateEmailFlowToken();
    await repository.createEmailVerification({
        user_id: userId,
        email: normalizeEmail(user.email),
        ...
        expires_at: new Date(Date.now() + ACCOUNT_DELETION_TOKEN_TTL_SECONDS * 1000),
    });
    await authEmail.send({
        to: user.email,
    ...
    return {
        message: 'Confirmation email sent. Your account is not scheduled until you confirm the link.',
        scheduled_at: scheduledAt.toISOString(),
    };
}
export async function confirmAccountDeletion(input, ipAddress, requestId) {
    const tokenHash = hashEmailFlowToken('account_deletion', input.token);
    const scheduledAt = new Date(Date.now() + ACCOUNT_DELETION_GRACE_SECONDS * 1000);
    let userId = null;
    await repository.withTransaction(async (client) => {
        const consumed = await repository.consumeEmailVerificationToken(tokenHash, 'account_deletion', client);
        if (!consumed) {
            throw new AuthError('Invalid or expired deletion confirmation token', AuthErrorCodes.EMAIL_VERIFICATION_INVALID, 400);
        }
        const scheduled = await repository.scheduleAccountDeletion(consumed.user_id, scheduledAt, client);
        if (!scheduled) {
            throw new AuthError('User not found', AuthErrorCodes.USER_NOT_FOUND, 404);
        }
        userId = scheduled.id;
...
export async function discoverSso(input) {
    ...
    if (user) {
        ...
        for (const id of identities) {
            if (isProviderConfigured(id.provider)) {
                linked.push(id.provider);
            }
        }
    }
    return {
...
export async function requestMfaRecovery(userId, input, ipAddress, requestId) {
    const user = await repository.findUserById(userId);
    if (!user) {
        throw new AuthError('User not found', AuthErrorCodes.USER_NOT_FOUND, 404);
    }
    ...
    return {
        message: 'Your MFA recovery request was recorded. Support will verify your identity before resetting MFA.',
    };
}
export async function listUserAuditEvents(targetUserId, adminId, isAdmin, query, ipAddress, requestId) {
    if (!isAdmin) {
...
export async function processDueAccountDeletions() {
    ...
    for (const user of due) {
        await repository.withTransaction(async (client) => {
            await repository.softDeleteUser(user.id, 'Scheduled account deletion', user.id, client);
            await repository.revokeAllUserSessions(user.id, 'Scheduled account deletion', client);
        });
