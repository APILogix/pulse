 ...
 */
import { authorizationCodeGrant, buildAuthorizationUrl, calculatePKCECodeChallenge, discovery, randomPKCECodeVerifier, randomState, } from 'openid-client';
import { env as config } from '../../config/env.js';
import { logAudit } from '../../shared/middleware/audit-logger.js';
import { identityLinkStateCache } from './cache.js';
import { getApiIdentityLinkCallbackUrl, getMicrosoftIssuer, isLinkableProvider, isProviderConfigured, } from './identity-link.config.js';
import * as repository from './repository.js';
import { AuthError, AuthErrorCodes } from './types.js';
import { normalizeEmail } from './utils.js';
function assertProviderConfigured(provider) {
    if (!isProviderConfigured(provider)) {
        throw new AuthError(`${provider} account linking is not configured on this deployment`, AuthErrorCodes.IDENTITY_PROVIDER_NOT_CONFIGURED, 503);
    }
}
async function buildOidcLinkConfig(provider) {
...
export async function startIdentityLink(userId, provider, ipAddress, requestId) {
    ...
    if (existing) {
        throw new AuthError(`${provider} is already linked to this account`, AuthErrorCodes.IDENTITY_ALREADY_LINKED, 409);
    }
...
export async function completeIdentityLink(callbackUrl, ipAddress, requestId) {
    const url = new URL(callbackUrl);
    const state = url.searchParams.get('state');
    const code = url.searchParams.get('code');
    if (!state || !code) {
        throw new AuthError('Missing state or authorization code', AuthErrorCodes.IDENTITY_LINK_FAILED, 400);
    }
    const flow = identityLinkStateCache.get(state);
    if (!flow) {
        throw new AuthError('Link session expired; start again from account settings', AuthErrorCodes.IDENTITY_LINK_FAILED, 400);
    }
    ...
    const collision = await repository.findLinkedIdentityByProviderSubject(flow.provider, providerSubject);
    if (collision && collision.user_id !== flow.userId) {
        throw new AuthError('This external account is already linked to another user', AuthErrorCodes.IDENTITY_ALREADY_LINKED, 409);
    }
...
export async function listUserLinkedIdentities(userId) {
...
export async function unlinkIdentity(userId, linkId, ipAddress, requestId) {
...
//# sourceMappingURL=identity-link.service.js.map