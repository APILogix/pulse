...
import { CreateUserSchema, LoginSchema, LoginMFAVerifySchema, UpdateUserSchema, DeleteUserSchema, ChangePasswordSchema, ForgotPasswordSchema, ResetPasswordSchema, MFASetupSchema, MFAVerifySetupSchema, MFAVerifySchema, BackupCodeVerificationSchema, AuthError, AuthErrorCodes, } from './types.js';
...
async function mfaRoutes(fastify) {
    ...
    fastify.delete('/mfa/devices/:id', {
        ...
        try {
            ...
            const body = (request.body || {});
            const { ip } = getClientInfo(request);
            await service.removeMFADevice(params.id, request.user.id, body.mfa_code, ip, request.id);
            return reply.status(204).send();
        }
        catch (error) {
            return handleAuthError(error, reply);
