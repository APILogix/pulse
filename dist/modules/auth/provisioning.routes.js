...
import { buildConfiguredCallbackUrl, getApiSocialLoginCallbackUrl, } from './oauth-callback.config.js';
...
export default async function provisioningRoutes(fastify) {
    ...
    fastify.get('/login/social/callback', { preHandler: [ssoCallbackRateLimit] }, async (request, reply) => {
        try {
            ...
            const ci = getClientInfo(request);
            const callbackUrl = buildConfiguredCallbackUrl(getApiSocialLoginCallbackUrl(), request.url);
