...
export async function registerStressTestRoute(fastify) {
    fastify.get('/stress-test', { config: { rateLimit: false } }, async (request, reply) => {
    ...
    fastify.get('/stress-test/async', { config: { rateLimit: false } }, async (request, reply) => {
        const query = request.query;
        const intensity = query.intensity || 'medium';
        const start = Date.now();
        const tasks = [
