...
import { ConflictError, ForbiddenError, NotFoundError, } from "./types.js";
...
export class OrganizationService {
    ...
    async createOrganization(data, ownerUserId, meta) {
    ...
    async listUserOrganizations(userId, pagination) {
    ...
    async getOrganization(orgId, userId, requiredRole) {
        const org = await this.requireOrganizationAccess(orgId, userId, requiredRole);
        return this.toOrganizationDto(org);
    }
    async updateOrganization(orgId, data, userId, meta) {
        ...
        if (data.billingName !== undefined)
            updatePayload.billingName = data.billingName;
        if (data.billingAddress !== undefined) {
            updatePayload.billingAddress = data.billingAddress;
        }
        if (data.dataRegion !== undefined)
            updatePayload.dataRegion = data.dataRegion;
        if (data.enforceSso !== undefined)
            updatePayload.enforceSso = data.enforceSso;
        if (data.enforceMfa !== undefined)
            updatePayload.enforceMfa = data.enforceMfa;
        ...
        if (data.ipAllowlist !== undefined)
            updatePayload.ipAllowlist = data.ipAllowlist;
    ...
    async deleteOrganization(orgId, userId, meta) {
    ...
    async restoreOrganization(orgId, userId, meta) {
    ...
    async getAuditLogs(orgId, userId, pagination) {
        // await this.requireOrganizationAccess(orgId, userId, "admin");
    ...
    async getPlan(orgId, userId) {
        const org = await this.requireOrganizationAccess(orgId, userId);
        return this.toPlanDto(org);
    }
    async upgradePlan(orgId, data, userId, meta) {
        const org = await this.requireOrganizationAccess(orgId, userId, "admin");
        if (org.ownerUserId !== userId) {
    ...
    async getSecuritySettings(orgId, userId) {
        const org = await this.requireOrganizationAccess(orgId, userId, "admin");
        return this.toSecuritySettingsDto(org);
    }
    async updateSecuritySettings(orgId, data, userId, meta) {
        // await this.requireOrganizationAccess(orgId, userId, "admin");
        const updated = await this.deps.repository.update(orgId, {
    ...
    async listMembers(orgId, userId, pagination) {
        await this.requireOrganizationAccess(orgId, userId);
        const result = await this.deps.repository.findMembersByOrgId(orgId, pagination);
        return this.mapPage(result, (row) => this.toMemberDto(row));
    }
    async getMember(orgId, targetUserId, requestingUserId) {
    ...
    async addMember(orgId, data, addedBy, meta) {
        ...
        await this.audit(orgId, addedBy, "org.member_invited", "organization", orgId, { memberUserId: data.userId, role: "member", method: "admin_add" }, meta);
    ...
    async removeMember(orgId, userId, removedBy, reason, meta) {
    ...
    async updateMemberRole(orgId, userId, newRole, updatedBy, meta) {
        const [org, target] = await Promise.all([
    ...
    async transferOwnership(orgId, toUserId, fromUserId, meta) {
    ...
    async leaveOrganization(orgId, userId, meta) {
        const org = await this.requireOrganizationAccess(orgId, userId);
        if (org.ownerUserId === userId) {
    ...
    async listInvitations(orgId, userId, pagination, status) {
        await this.requireOrganizationAccess(orgId, userId, "admin");
        const result = await this.deps.repository.findInvitationsByOrgId(orgId, pagination, status);
        return this.mapPage(result, (row) => this.toInvitationDto(row));
    }
    async inviteMember(orgId, data, invitedBy, meta) {
    ...
    async acceptInvitation(token, userId, userEmail, meta) {
        const tokenHash = createHash("sha256").update(token).digest("hex");
        const invitation = await this.deps.repository.findInvitationByTokenHash(tokenHash);
    ...
    async declineInvitation(invitationId, userId, meta) {
    ...
    async resendInvitation(invitationId, userId, meta) {
        const invitation = await this.deps.repository.findInvitationById(invitationId);
        if (!invitation) {
            throw new NotFoundError("Invitation");
        }
    ...
    async revokeInvitation(invitationId, revokedBy, meta) {
        const invitation = await this.deps.repository.findInvitationById(invitationId);
        if (!invitation) {
            throw new NotFoundError("Invitation");
        }
    ...
    async audit(orgId, userId, action, resourceType, resourceId, metadata, meta) {
