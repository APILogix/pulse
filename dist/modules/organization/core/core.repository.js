...
export class CoreRepository extends BaseRepository {
    ...
    async restoreOrg(id) {
        const r = await this.db.query(`UPDATE organizations SET status='active',updated_at=NOW() WHERE id=$1 AND deleted_at IS NULL
    ...
    async transferOwnership(orgId, oldOwnerId, newOwnerId) {
        return this.withTransaction(async (client) => {
            const r = await client.query(`UPDATE organizations SET owner_user_id=$1,updated_at=NOW() WHERE id=$2 AND owner_user_id=$3 AND deleted_at IS NULL`, [newOwnerId, orgId, oldOwnerId]);
            if (r.rowCount === 0)
                throw new ConflictError("Invalid ownership transfer");
    ...
    async updateSettings(orgId, data) {
        ...
        if (cols.length === 0)
            throw new ConflictError("No fields to update");
        vals.push(orgId);
        const r = await this.db.query(`UPDATE organization_settings SET ${cols.join(',')},updated_at=NOW() WHERE org_id=$${vals.length}
    ...
    async listUserOrganizations(userId, q) {
        ...
        const r = await this.db.query(`SELECT o.id,o.name,o.slug,o.logo_url,om.role,o.status,o.created_at
