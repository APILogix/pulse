...
export class OrganizationRepository {
    ...
    async softDeleteOrg(id) {
        const r = await this.db.query(`UPDATE organizations SET deleted_at=NOW(),status='archived' WHERE id=$1 AND deleted_at IS NULL`, [id]);
        if (r.rowCount === 0)
            throw new NotFoundError("Organization");
    }
    async archiveOrg(id) {
    ...
    async createAuditLog(entry) {
        await this.db.query(`INSERT INTO audit_logs (org_id,actor_user_id,actor_email,actor_ip,actor_user_agent,actor_session_id,action,entity_type,entity_id,entity_name,request_id,http_method,endpoint,old_values,new_values,changed_fields,status,failure_reason,is_sensitive,metadata)
