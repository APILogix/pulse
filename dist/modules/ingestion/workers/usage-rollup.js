...
import { INGEST_USAGE_ROLLUP_QUEUE, } from '../queue/ingest-queues.js';
...
function monthPartition(offsetMonths) {
    const now = new Date();
...
export class UsageRollup {
    ...
    async start() {
        await pgboss.work(INGEST_USAGE_ROLLUP_QUEUE, { localConcurrency: 1, batchSize: 1 }, (async () => {
            await this.run();
        }));
        await pgboss.schedule(INGEST_USAGE_ROLLUP_QUEUE, env.INGESTION_USAGE_ROLLUP_CRON, { triggeredAt: new Date().toISOString() }, { singletonKey: 'usage-rollup' });
    ...
    async run() {
        ...
        try {
            ...
            for (const offset of [0, 1]) {
                ...
                await client.query(`CREATE TABLE IF NOT EXISTS ${p.name} PARTITION OF usage_daily_counters FOR VALUES FROM ('${p.from}') TO ('${p.to}')`);
            }
            // (b) Atomic extract of the billing counters.
            const extracted = await client.query(`DELETE FROM usage_counter_staging
         ...
         RETURNING org_id, project_id, counter_type, increment_by`);
            ...
            for (const row of extracted.rows) {
                ...
                if (column)
                    acc.byColumn.set(column, (acc.byColumn.get(column) ?? 0) + n);
                acc.total += n;
                eventsTotal += n;
            }
        ...
        this.log.info({ stagingRows, events: eventsTotal, orgs: orgTotals.size, projects: scopes.size, ms: Date.now() - startedAt }, 'Usage rollup complete');
    }
}
//# sourceMappingURL=usage-rollup.js.map