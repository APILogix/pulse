...
export class PgQueue {
    ...
    async enqueueBulk(jobs, client) {
        ...
        for (const j of jobs) {
            ...
            params.push(this.queue, j.jobType, j.priority ?? 100, j.orgId ?? null, j.projectId ?? null, JSON.stringify(j.payload), j.dedupeKey ?? null, j.delayMs ?? 0, j.maxAttempts ?? 5);
        }
    ...
    async complete(jobId) {
       ...
       WHERE id = $1 AND state = 'active'`, [jobId]);
    }
    /**
    ...
    async fail(job, errorMessage) {
        if (job.attempts >= job.maxAttempts) {
           ...
           WHERE id = $1`, [job.id, errorMessage.slice(0, 4000)]);
        ...
        await this.pool.query(`UPDATE ingestion_jobs
         SET state = 'pending',
             ...
             locked_until = NULL,
             locked_by = NULL
       WHERE id = $1`, [job.id, backoff + jitter, errorMessage.slice(0, 4000)]);
        return 'retried';
    }
    ...
    async replayDeadLetter(dlqId) {
        ...
        try {
         ...
         RETURNING id`, [row.queue, row.job_type, row.org_id, row.project_id, JSON.stringify(row.payload), row.dedupe_key]);
            await client.query(`UPDATE ingestion_dead_letter_jobs SET replayed_at = NOW() WHERE id = $1`, [dlqId]);
            await client.query('COMMIT');
