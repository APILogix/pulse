 ...
 */
async function resolvePresetActor(pool, orgId) {
    const owner = await pool.query(`SELECT owner_user_id FROM organizations WHERE id = $1 AND deleted_at IS NULL`, [orgId]);
    if (owner.rows[0])
        return owner.rows[0].owner_user_id;
    const member = await pool.query(`SELECT user_id
 ...
 */
export async function seedDefaultPresetsForOrg(pool, orgId) {
    ...
    try {
        ...
        for (const preset of DEFAULT_ALERT_PRESETS) {
            const inserted = await client.query(`INSERT INTO alert_rules
         ...
         RETURNING id`, [
            ...
            if (!rule)
                continue; // preset already exists for this org
         ...
         VALUES ($1, 'notify'::alert_action_type, 100, 0, $2, TRUE)`, [rule.id, JSON.stringify({ channel: 'in_app', audience: 'org_members' })]);
            seeded += 1;
        }
