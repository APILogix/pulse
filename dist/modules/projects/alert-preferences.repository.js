...
export class AlertPreferencesRepository {
    ...
    async getPreferences(projectId, userId) {
        const res = await pool.query(`SELECT ${PREF_COLUMNS} FROM project_member_alert_preferences WHERE project_id = $1 AND user_id = $2`, [projectId, userId]);
        return res.rows.map(this.mapRow);
    }
    async createPreference(projectId, userId, routeId, client = pool) {
      ...
      `, [projectId, userId, routeId]);
        if (!res.rows.length) {
            ...
            const existing = await client.query(`SELECT ${PREF_COLUMNS} FROM project_member_alert_preferences WHERE project_id = $1 AND user_id = $2 AND route_id = $3`, [projectId, userId, routeId]);
            return this.mapRow(existing.rows[0]);
        }
        return this.mapRow(res.rows[0]);
    }
    async updatePreference(prefId, projectId, userId, dto) {
        const fields = [];
        const values = [];
        let idx = 1;
    ...
    async bulkSubscribe(projectId, routeId, userIds, client = pool) {
        if (userIds.length === 0)
            return;
