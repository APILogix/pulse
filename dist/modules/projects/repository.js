...
export class ProjectsRepository {
    ...
    async createDefaultEnvironments(project, createdBy, client) {
        const envs = [];
        for (const environment of DEFAULT_PROJECT_ENVIRONMENTS) {
            envs.push(await this.createEnvironment({
    ...
    buildProjectAssignments(input) {
        ...
        if (input.productionApiPrefix !== undefined)
            set("production_api_prefix", input.productionApiPrefix);
        if (input.developmentApiPrefix !== undefined)
            set("development_api_prefix", input.developmentApiPrefix);
        if (input.stagingApiPrefix !== undefined)
            set("staging_api_prefix", input.stagingApiPrefix);
        if (input.rateLimitPerSecond !== undefined)
            set("rate_limit_per_second", input.rateLimitPerSecond);
        if (input.rateLimitPerMinute !== undefined)
            set("rate_limit_per_minute", input.rateLimitPerMinute);
        ...
        if (input.ipBlocklist !== undefined)
            set("ip_blocklist", input.ipBlocklist);
        if (input.geoRestrictionEnabled !== undefined)
            set("geo_restriction_enabled", input.geoRestrictionEnabled);
        if (input.allowedCountries !== undefined)
            set("allowed_countries", input.allowedCountries);
        if (input.alertEmail !== undefined)
            set("alert_email", input.alertEmail);
        if (input.alertWebhookUrl !== undefined)
            set("alert_webhook_url", input.alertWebhookUrl);
        if (input.alertOnErrorRateThreshold !== undefined)
            set("alert_on_error_rate_threshold", input.alertOnErrorRateThreshold);
        if (input.alertOnLatencyThresholdMs !== undefined)
            set("alert_on_latency_threshold_ms", input.alertOnLatencyThresholdMs);
        if (input.metadata !== undefined)
            set("metadata", JSON.stringify(input.metadata));
        if (input.settings !== undefined)
            set("settings", JSON.stringify(input.settings));
    ...
    prefixedProjectRow(row) {
        return {
            ...
            production_api_prefix: row.p_production_api_prefix,
            development_api_prefix: row.p_development_api_prefix,
            staging_api_prefix: row.p_staging_api_prefix,
            rate_limit_per_second: row.p_rate_limit_per_second,
            rate_limit_per_minute: row.p_rate_limit_per_minute,
            ...
            require_https: row.p_require_https,
            ip_allowlist: row.p_ip_allowlist,
            ip_blocklist: row.p_ip_blocklist,
            geo_restriction_enabled: row.p_geo_restriction_enabled,
            allowed_countries: row.p_allowed_countries,
            alert_email: row.p_alert_email,
            alert_webhook_url: row.p_alert_webhook_url,
            alert_on_error_rate_threshold: row.p_alert_on_error_rate_threshold,
            alert_on_latency_threshold_ms: row.p_alert_on_latency_threshold_ms,
            metadata: row.p_metadata,
            settings: row.p_settings,
