...
export class AnalyticsQueryBuilder {
    ...
    orderByClause = '';
    limitClause = '';
    constructor(tableKey, orgId) {
        const resolved = ANALYTICS_TABLES[tableKey];
        if (!resolved)
            throw new Error(`Unknown analytics table: ${tableKey}`);
        this.table = resolved;
    ...
    whereTimeRange(start, end, column = 'timestamp') {
    ...
    whereTrue(column, enabled) {
        if (enabled)
            this.conditions.push(`${column} = true`);
        return this;
    }
    whereSearch(column, query) {
        if (!query)
            return this;
     ...
     */
    timeSeries(bucket, extraSelects = '') {
