...
function getRouteIds(request) {
    ...
    return {
        orgId: (typeof params.orgId === "string" && params.orgId) ||
...
export async function requireOrgAccess(request, _reply) {
...
export async function requireProjectAdmin(request, _reply) {
    const { orgId, projectId } = getRouteIds(request);
    const userId = request.user?.id;
    if (!orgId || !projectId || !userId) {
        throw new ProjectError("INSUFFICIENT_PERMISSIONS", "Project context is required", 403);
    }
    ...
    if (!membership || !membership.is_active || !hasRequiredRole(membership.role, "admin")) {
        throw new ProjectError("INSUFFICIENT_PERMISSIONS", "Project admin access required", 403);
    }
}
//# sourceMappingURL=requireorg.js.map