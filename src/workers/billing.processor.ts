...
let invoiceTimer: NodeJS.Timeout | null = null;
let dunningTimer: NodeJS.Timeout | null = null;
let usageRollupTimer: NodeJS.Timeout | null = null;
...
async function runJob(
  pool: Pool,
  jobName: string,
  handler: () => Promise<{ processed: number; succeeded: number; failed: number; metadata?: Record<string, unknown> }>
): Promise<void> {
  if (jobLocks[jobName]) return;
  jobLocks[jobName] = true;

  const startedAt = Date.now();
  const runKey = `${jobName}:${new Date().toISOString()}`;
  const insertResult = await pool.query<{ id: string }>(
    `INSERT INTO billing_job_runs (job_name, run_key, status, started_at)
    ...
    await pool.query(
      `UPDATE billing_job_runs
       SET status = 'failed',
           ...
           error_summary = $2
       WHERE id = $3`,
      [durationMs, error instanceof Error ? error.message : 'Unknown billing job error', runId]
    );
...
async function runInvoiceCycle(pool: Pool): Promise<{ processed: number; succeeded: number; failed: number }> {
  ...
  const dueResult = await pool.query<{
    ...
    `SELECT org_id, plan_id, current_period_start, current_period_end, net_terms_days, cancel_at_period_end
...
async function runDunning(pool: Pool): Promise<{ processed: number; succeeded: number; failed: number }> {
  const overdueResult = await pool.query<{ id: string; org_id: string }>(
    `SELECT id, org_id
  ...
  for (const invoice of overdueResult.rows) {
    try {
      await pool.query(
        ...
        [SubscriptionStatus.PAST_DUE, invoice.org_id, SubscriptionStatus.CANCELED]
      );
      succeeded += 1;
    } catch {
      failed += 1;
...
async function runUsageRollup(pool: Pool): Promise<{ processed: number; succeeded: number; failed: number }> {
  const counters = await pool.query<{
    org_id: string;
    api_requests_this_period: string;
    metrics_ingested_this_period: string;
    storage_gb_this_period: string;
    notifications_sent_this_period: string;
  }>(
    `SELECT org_id, api_requests_this_period, metrics_ingested_this_period, storage_gb_this_period, notifications_sent_this_period
    ...
    for (const snapshot of snapshots) {
      try {
        await pool.query(
          ...
          [counter.org_id, snapshot.type, snapshot.name, dayStart, dayEnd, snapshot.value]
        );
        succeeded += 1;
      } catch {
        failed += 1;
...
async function runWebhookReconciliation(pool: Pool): Promise<{ processed: number; succeeded: number; failed: number }> {
  const staleEvents = await pool.query<{ id: string; retry_count: number }>(
...
export function startBillingWorker(pool: Pool): void {
  if (invoiceTimer || dunningTimer || usageRollupTimer || webhookTimer) return;
...
export function stopBillingWorker(): void {
  ...
  if (usageRollupTimer) clearInterval(usageRollupTimer);
