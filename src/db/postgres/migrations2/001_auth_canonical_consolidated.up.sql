...
--    Purposes: email_verification, password_reset, mfa_disable, email_change,
...
-- ----------------------------------------------------------------------------
-- 8) AUDIT LOGS  (one canonical definition — matches audit-logger.ts exactly)
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 10) LINKED IDENTITIES (phase 6) — OAuth social login / linking
-- ----------------------------------------------------------------------------
...
--
-- #1 repository.ts:updateUserEmail currently issues:
--      UPDATE users SET email = $2, email_hash = $3 ...
--    email_hash is GENERATED ALWAYS AS ... STORED, so this UPDATE throws
--    "cannot insert into column email_hash". The repository must NOT write
--    email_hash; the generated column recomputes itself from email. This
--    schema documents that constraint; fix is in repository.ts.
--
