-- ============================================================================
-- 006_add_organization_module.up.sql
-- ----------------------------------------------------------------------------
...
--
--   * organization_email_outbox      — durable, retrying email queue for org
--                                       and project emails (mirrors the auth
--                                       module's auth_email_outbox). Drained by
--                                       the org-email worker. No Redis.
--   * organization_alert_thresholds  — per-org (optionally per-project) latency
...
-- so every org audit write failed — fixed here).
--
...
-- ----------------------------------------------------------------------------
-- 7) ORGANIZATION AUDIT LOGS  (enterprise shape — repository writes here)
...
--    (existing simple org-scoped key store — retained for the current module.
...
CREATE TABLE IF NOT EXISTS organization_alert_thresholds (
    ...
    -- Which percentiles actually trigger an email. p95/p99 on by default —
