...
--
-- Conventions match 002_connectors_create_notification_schema.up.sql:
--   * Enums guarded with DO/IF NOT EXISTS so re-runs are no-ops.
--   * Partial UNIQUE INDEXes (â€¦ WHERE deleted_at IS NULL) instead of UNIQUE
--     constraints on (org, name, deleted_at) â€” the latter still permits
...
-- ----------------------------------------------------------------------------
-- 15.1) Foreign keys that may depend on later migrations / later tables
-- ----------------------------------------------------------------------------
DO $$
BEGIN
...
-- ----------------------------------------------------------------------------
-- 17) ROW LEVEL SECURITY (INTENTIONALLY DISABLED â€” see migration header)
-- ----------------------------------------------------------------------------
