-- ============================================================================
-- 008_add_projects_module.up.sql
-- ----------------------------------------------------------------------------
...
--   * projects                — rate-limit config, ingestion config, security
--                               (ip allow/block, geo, https), alerting config,
--                               staging prefix, settings, soft-delete actor.
--   * project_environments    — per-environment config overrides (dev/stg/prod).
--   * project_api_keys         — key_type, lifecycle status, rotation lineage +
...
--   * project_api_key_usage    — per-key per-day/hour usage analytics rollups.
--
...
--
-- Depends on: 006_add_organization_module (organizations, users).
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS projects (
    ...
    -- in-process LRU rate limiter — NOT Redis). Stored here as the source of
...
-- repository — projects and API keys are org-owned resources, so they share
