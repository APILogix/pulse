...
--
-- DESIGN NOTE (Option A â€” extend, do not duplicate):
--   This migration EXTENDS the existing canonical MFA schema from
--   001_auth_create_core_schema.up.sql. It deliberately does NOT create a
...
-- app.current_org_id (see notes in 004_analytics_create_core_schema.up.sql and the
-- BUGFIX #4 note in 001_auth_create_core_schema.up.sql).
-- ============================================================================
...
-- ----------------------------------------------------------------------------
-- 1) user_mfa_devices â€” columns for "try another way" + per-device lockout
-- ----------------------------------------------------------------------------

-- Human-readable masked hint shown in the "try another way" picker, e.g.
-- "+1 â€¢â€¢â€¢â€¢ 1234", "jâ€¢â€¢â€¢@example.com", "Google Authenticator", "YubiKey 5".
...
-- ----------------------------------------------------------------------------
-- 2) sms_mfa_otps â€” SMS one-time codes (mirrors email_mfa_otps)
...
-- ----------------------------------------------------------------------------
-- 3) organization_settings â€” organization-level MFA policy
...
COMMENT ON COLUMN user_mfa_devices.display_hint IS
  'Masked, human-readable hint for the "try another way" picker (e.g. "+1 â€¢â€¢â€¢ 1234").';
