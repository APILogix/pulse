...
export class BillingService {
  ...
  async getSubscriptionHistory(orgId: string): Promise<ServiceResponse<Array<Record<string, unknown>>>> {
  ...
  async getUsageOverview(orgId: string): Promise<ServiceResponse<{
    orgId: string;
    periodStart: Date;
    periodEnd: Date;
    generatedAt: Date;
      ...
      data: {
        ...
        activity: activity
          .map((record) => ({
            ...
            aiAnalyses: Number((record.details as { aiAnalysesCount?: number } | null)?.aiAnalysesCount ?? 0),
          }))
  ...
  async getDetailedUsage(
    ...
    if (params.granularity) options.granularity = params.granularity;
