...
import type {BillingPlan,
  ...
  UsageCounter,
  QuotaRequest,
  Coupon,ServiceResponse} from "./types.js"
import {
...
export class BillingRepository {
  ...
  async getAllPlans(includeHidden: boolean = false): Promise<BillingPlan[]> {
  ...
  async getPlanByTier(tier: PlanTier): Promise<BillingPlan | null> {
    const result = await this.pool.query(
      'SELECT * FROM billing_plans WHERE tier = $1 AND is_active = true LIMIT 1',
      [tier]
    );
    return result.rows.length > 0 ? this.mapPlanFromDb(result.rows[0]) : null;
  }
  ...
  async getOrganizationById(orgId: string): Promise<{
    id: string;
    name: string;
    slug: string;
    ownerUserId: string;
  ...
  async getOrganizationMembership(
    ...
    const result = await this.pool.query(
      `SELECT role, permissions, is_active, joined_at
  ...
  async updateSubscriptionStatus(
    ...
    const db = client || this.pool;
    await db.query(
      'UPDATE organization_billing SET status = $1, updated_at = NOW() WHERE org_id = $2',
      [status, orgId]
    );
  }

  private mapBillingFromDb(row: any): OrganizationBilling {
    return {
      id: row.id,
      orgId: row.org_id,
      planId: row.plan_id,
  ...
  async getPaymentMethods(orgId: string): Promise<PaymentMethod[]> {
    const result = await this.pool.query(
      'SELECT * FROM organization_payment_methods WHERE org_id = $1 AND is_active = true ORDER BY is_default DESC, created_at DESC',
      [orgId]
    );
    return result.rows.map(this.mapPaymentMethodFromDb);
  }

  async getPaymentMethodById(id: string, orgId: string): Promise<PaymentMethod | null> {
    const result = await this.pool.query(
      'SELECT * FROM organization_payment_methods WHERE id = $1 AND org_id = $2',
      [id, orgId]
    );
  ...
  async getDefaultPaymentMethod(orgId: string): Promise<PaymentMethod | null> {
    const result = await this.pool.query(
      'SELECT * FROM organization_payment_methods WHERE org_id = $1 AND is_default = true AND is_active = true LIMIT 1',
      [orgId]
    );
    return result.rows.length > 0 ? this.mapPaymentMethodFromDb(result.rows[0]) : null;
  }
  ...
  async createPaymentMethod(
    ...
    const existingCount = await db.query(
      'SELECT COUNT(*) FROM organization_payment_methods WHERE org_id = $1 AND is_active = true',
      [paymentMethod.orgId]
    );
  ...
  async setDefaultPaymentMethod(
    ...
    await db.query(
      'UPDATE organization_payment_methods SET is_default = false WHERE org_id = $1',
      [orgId]
    );
    
    await db.query(
      'UPDATE organization_payment_methods SET is_default = true WHERE id = $1 AND org_id = $2',
  ...
  async updatePaymentMethod(
    ...
    const result = await this.pool.query(
      `UPDATE organization_payment_methods SET
  ...
  async deletePaymentMethod(id: string, orgId: string): Promise<void> {
    await this.pool.query(
      'UPDATE organization_payment_methods SET is_active = false, updated_at = NOW() WHERE id = $1 AND org_id = $2',
      [id, orgId]
    );
  }
  ...
  async getInvoices(
    ...
    const countResult = await this.pool.query(
      `SELECT COUNT(*) FROM organization_invoices ${whereClause}`,
      values
    );
  ...
  async getInvoiceById(id: string, orgId: string): Promise<Invoice | null> {
    const result = await this.pool.query(
      'SELECT * FROM organization_invoices WHERE id = $1 AND org_id = $2',
      [id, orgId]
    );
    return result.rows.length > 0 ? this.mapInvoiceFromDb(result.rows[0]) : null;
  }

  async getInvoiceByNumber(invoiceNumber: string, orgId: string): Promise<Invoice | null> {
    const result = await this.pool.query(
      'SELECT * FROM organization_invoices WHERE invoice_number = $1 AND org_id = $2',
      [invoiceNumber, orgId]
    );
    return result.rows.length > 0 ? this.mapInvoiceFromDb(result.rows[0]) : null;
  }
  ...
  async createInvoice(
    ...
    const billing = await db.query(
      'SELECT invoice_prefix, next_invoice_number FROM organization_billing WHERE org_id = $1 FOR UPDATE',
      [invoice.orgId]
    );
    ...
    await db.query(
      'UPDATE organization_billing SET next_invoice_number = next_invoice_number + 1 WHERE org_id = $1',
      [invoice.orgId]
    );
  ...
  async getUsageRecords(
    ...
    const result = await this.pool.query(
      `SELECT * FROM organization_usage ${whereClause} ORDER BY period_start DESC`,
      values
    );
  ...
  async getUsageCounter(orgId: string): Promise<UsageCounter | null> {
    const result = await this.pool.query(
      'SELECT * FROM organization_usage_counters WHERE org_id = $1',
      [orgId]
    );
  ...
  async incrementUsageCounter(
    orgId: string,
    metric: keyof Omit<UsageCounter, 'orgId' | 'currentPeriodStart' | 'lastUpdatedAt' | 'limitWarning80SentAt' | 'limitWarning100SentAt' | 'updatedAt'>,
    amount: number = 1,
    client?: PoolClient
  ): Promise<void> {
    ...
    await db.query(
      `INSERT INTO organization_usage_counters (org_id, current_period_start, ${column})
  ...
  private mapUsageRecordFromDb(row: any): UsageRecord {
    return {
      id: row.id,
      orgId: row.org_id,
  ...
  private mapUsageCounterFromDb(row: any): UsageCounter {
    return {
      orgId: row.org_id,
      currentPeriodStart: row.current_period_start,
  ...
  async incrementCouponUsage(code: string, client?: PoolClient): Promise<void> {
    const db = client || this.pool;
    await db.query(
  ...
  private mapCouponFromDb(row: any): Coupon {
    return {
      id: row.id,
      code: row.code,
      description: row.description,
  ...
  async getQuotaRequests(orgId: string): Promise<QuotaRequest[]> {
    const result = await this.pool.query(
      'SELECT * FROM quota_requests WHERE org_id = $1 ORDER BY created_at DESC',
      [orgId]
    );
    return result.rows.map(this.mapQuotaRequestFromDb);
  }

  private mapQuotaRequestFromDb(row: any): QuotaRequest {
    return {
      id: row.id,
      orgId: row.org_id,
