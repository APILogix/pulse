...
export class BillingRepository {
  ...
  async getAllPlans(includeHidden = false): Promise<BillingPlan[]> {
  ...
  async getPlanById(planId: string, db: Db = this.db): Promise<BillingPlan | null> {
  ...
  async getPlanByTier(tier: PlanTier, db: Db = this.db): Promise<BillingPlan | null> {
  ...
  async getOrganizationBilling(orgId: string, db: Db = this.db): Promise<OrganizationBilling | null> {
    const result = await db.query(
      `SELECT ${prefixColumns(SUBSCRIPTION_COLUMNS, 's')}
  ...
  async getOrganizationBillingForUpdate(orgId: string, db: PoolClient): Promise<OrganizationBilling | null> {
    const result = await db.query(
  ...
  async updateOrganizationBilling(
    ...
    if (updates.planId !== undefined) add('plan_id', updates.planId);
    if (updates.status !== undefined) add('status', updates.status);
    if (updates.billingProvider !== undefined) add('billing_provider', updates.billingProvider);
    if (updates.providerCustomerId !== undefined) add('provider_customer_id', updates.providerCustomerId);
    if (updates.providerSubscriptionId !== undefined) add('provider_subscription_id', updates.providerSubscriptionId);
    if (updates.billingInterval !== undefined) add('billing_interval', updates.billingInterval);
    if (updates.currentPeriodStart !== undefined) add('current_period_start', updates.currentPeriodStart);
    if (updates.currentPeriodEnd !== undefined) add('current_period_end', updates.currentPeriodEnd);
    if (updates.trialStart !== undefined) add('trial_start', updates.trialStart);
    if (updates.trialEnd !== undefined) add('trial_end', updates.trialEnd);
    if (updates.cancelAtPeriodEnd !== undefined) add('cancel_at_period_end', updates.cancelAtPeriodEnd);
    if (updates.canceledAt !== undefined) add('canceled_at', updates.canceledAt);
    if (updates.seats !== undefined) add('seats', updates.seats);
  ...
  async listSubscriptionEvents(orgId: string): Promise<Array<Record<string, unknown>>> {
    const result = await this.db.query(
  ...
  async getInvoices(
    orgId: string,
    options: { status?: InvoiceStatus; limit?: number; offset?: number; startDate?: Date; endDate?: Date } = {},
  ...
  async getInvoiceById(id: string, orgId: string): Promise<Invoice | null> {
  ...
  async createInvoice(invoice: Partial<Invoice>, db: Db = this.db): Promise<Invoice> {
    const result = await db.query(
  ...
  async updateInvoiceStatus(
    ...
    paymentDetails?: { paidAt: Date; paymentIntentId: string; amountPaid: number },
    db: Db = this.db,
  ): Promise<Invoice> {
    const result = await db.query(
      `UPDATE invoices
       ...
       WHERE id = $4
       RETURNING ${INVOICE_COLUMNS}`,
      [status, paymentDetails?.paidAt ?? null, paymentDetails?.amountPaid ?? null, id],
    );
    return mapInvoiceFromDb(result.rows[0]);
  }

  async getUpcomingInvoice(orgId: string): Promise<Partial<Invoice> | null> {
  ...
  async getUsageRecords(
    orgId: string,
    options: { startDate?: Date; endDate?: Date; granularity?: 'hourly' | 'daily' | 'monthly'; metricType?: UsageMetricType } = {},
  ): Promise<UsageRecord[]> {
    const values: unknown[] = [orgId];
    let i = 2;
    const where = ['org_id = $1'];
    ...
    const result = await this.db.query(
      `SELECT ${USAGE_DAILY_COUNTER_COLUMNS}
  ...
  async getUsageCounter(orgId: string): Promise<UsageCounter | null> {
    const result = await this.db.query(
      `SELECT
         org_id,
  ...
  async getCouponByCode(code: string, db: Db = this.db): Promise<Coupon | null> {
    const result = await db.query(
  ...
  async redeemCoupon(couponId: string, orgId: string, db: Db = this.db): Promise<void> {
    await db.query(
      `INSERT INTO coupon_redemptions (coupon_id, org_id)
       VALUES ($1, $2)
       ON CONFLICT (coupon_id, org_id) DO NOTHING`,
      [couponId, orgId],
    );
    await db.query(
      `UPDATE coupons
  ...
  async incrementCouponUsage(code: string, db: Db = this.db): Promise<void> {
    await db.query(
      `UPDATE coupons
       SET redemption_count = redemption_count + 1,
           updated_at = NOW()
  ...
  async createQuotaRequest(request: Partial<QuotaRequest>): Promise<QuotaRequest> {
    const result = await this.db.query(
      `INSERT INTO quota_requests (org_id, quota_type, requested_limit, current_limit, reason)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING ${QUOTA_REQUEST_COLUMNS}`,
      [request.orgId, request.quotaType, request.requestedLimit, request.currentLimit, request.reason],
    );
    return mapQuotaRequestFromDb(result.rows[0]);
  }

  async getQuotaRequests(orgId: string): Promise<QuotaRequest[]> {
    const result = await this.db.query(
      `SELECT ${QUOTA_REQUEST_COLUMNS} FROM quota_requests WHERE org_id = $1 ORDER BY created_at DESC`,
      [orgId],
    );
    return result.rows.map(mapQuotaRequestFromDb);
  }

  async getPaymentMethods(_orgId: string): Promise<PaymentMethod[]> {
    return [];
  }

  async getPaymentMethodById(_id: string, _orgId: string): Promise<PaymentMethod | null> {
    return null;
  }
  ...
  async setDefaultPaymentMethod(_orgId: string, _paymentMethodId: string): Promise<void> {}
  async updatePaymentMethod(_id: string, orgId: string, updates: Partial<PaymentMethod>): Promise<PaymentMethod> {
    return this.createPaymentMethod({ ...updates, orgId });
  }
  async deletePaymentMethod(_id: string, _orgId: string): Promise<void> {}
}

function mapPlanFromDb(row: any): BillingPlan {
...
function mapSubscriptionFromDb(row: any): OrganizationBilling {
  return {
    id: row.id,
    orgId: row.org_id,
    planId: row.plan_id,
...
function mapUsageRecordFromDb(row: any, metricType: UsageMetricType): UsageRecord {
...
function mapQuotaRequestFromDb(row: any): QuotaRequest {
  return {
    id: row.id,
    orgId: row.org_id,
