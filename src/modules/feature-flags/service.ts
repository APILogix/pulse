 ...
 */
flagCache.invalidate = (key: string, ctx?: FeatureFlagContext): void => {
  if (ctx) {
  ...
  for (const k of flagCache.keys()) {
    if (k.startsWith(prefix)) flagCache.delete(k);
  }
};
 ...
 */
export async function isEnabled(key: string, ctx: FeatureFlagContext = {}): Promise<boolean> {
  const cacheKey = flagCacheKey(key, ctx);
  const cached = flagCache.get(cacheKey);
  if (cached !== undefined) return cached.enabled;

  const r = await pool.query<{ scope: string; enabled: boolean }>(
  ...
  const byScope = new Map<string, boolean>(r.rows.map((row) => [row.scope, row.enabled]));
...
export async function setFlag(input: SetFlagInput): Promise<void> {
  ...
  try {
    ...
    const inserted = await client.query<{ id: string }>(
      `INSERT INTO feature_flags (key, scope, scope_id, enabled, payload, description)
       VALUES ($1, $2, $3, $4, $5, $6)
