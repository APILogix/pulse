...
export function assertSafeHttpsUrl(raw: string): URL {
  ...
  if (isIP(host) && isPrivateIp(host)) throw new Error('Private/link-local IPs are not allowed');
  return u;
}

/** Async DNS check — call from the service layer at create/update time. */
export async function assertPubliclyResolvable(u: URL): Promise<void> {
  if (isIP(u.hostname)) return; // literal IP already checked
