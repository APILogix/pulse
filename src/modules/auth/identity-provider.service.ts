...
import { apiBaseUrl, getApiProviderAuthorizeUrl } from './oauth-callback.config.js';
import {
  identityLinkStateCache,
  socialLoginStateCache,
} from './cache.js';
import {
  isLinkableProvider,
...
import { AuthError, AuthErrorCodes, type SocialLoginInput } from './types.js';
...
function readCallbackState(state: string): CallbackStateResult | null {
...
function assertProviderReady(provider: string): asserts provider is LinkableProvider {
...
function buildAuthorizeUrl(provider: LinkableProvider, flow: 'login' | 'link', state: string): string {
  const url = new URL(getApiProviderAuthorizeUrl(provider, flow));
  url.searchParams.set('state', state);
  return url.toString();
}

export function frontendAuthCallbackUrl(): string {
  const base = (apiBaseUrl() || '').replace(/\/+$/, '');
  return `${base}/auth/callback`;
}

export function frontendIdentityProvidersUrl(): string {
  const base = (apiBaseUrl() || '').replace(/\/+$/, '');
...
export function consumeCallbackState(state: string): CallbackStateResult {
...
export function resolveCallbackState(state: string): CallbackStateResult {
  const flow = readCallbackState(state);
  if (flow) {
    return flow;
  }
...
export async function completeSocialLogin(
  profile: PassportSocialProfile,
  flow: CallbackStateResult,
  ipAddress: string,
  userAgent: string,
  requestId: string,
): Promise<{
...
export async function completeIdentityLink(
  ...
  ipAddress: string,
  requestId: string,
): Promise<{ id: string; provider: LinkableProvider; linked_at: Date }> {
  const userId = flow.userId;
  if (!userId) {
    throw new AuthError(
      'Invalid identity link session',
  ...
  const existingByProvider = await repository.findLinkedIdentityByUserProvider(
    userId,
    profile.provider,
  );
  if (existingByProvider) {
    if (existingByProvider.provider_subject !== profile.subject) {
      throw new AuthError(
        'Provider already linked to a different identity',
        AuthErrorCodes.IDENTITY_ALREADY_LINKED,
        409,
      );
    }
...
export async function listIdentityProviders(userId: string) {
...
export async function unlinkIdentityProvider(
  ...
  if (!user.password_hash && links.length <= 1) {
    throw new AuthError(
      'Cannot remove the last login method from a passwordless account',
