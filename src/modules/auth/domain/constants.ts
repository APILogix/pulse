...
export function buildPasswordHistory(currentHistory: unknown, currentPasswordHash: string | null): string[] {
  const history = Array.isArray(currentHistory) ? currentHistory.filter((entry): entry is string => typeof entry === 'string') : [];
  const ordered = [currentPasswordHash, ...history].filter((entry): entry is string => Boolean(entry));
  const seen = new Set<string>();
  const uniq: string[] = [];
  for (const h of ordered) { if (!seen.has(h)) { seen.add(h); uniq.push(h); } }
  return uniq.slice(0, 5);
}
export function lockoutDurationSeconds(failedAttempts: number): number {
  if (failedAttempts < 5) return 0;
  if (failedAttempts < 7) return 60;
  if (failedAttempts < 9) return 5 * 60;
  if (failedAttempts < 11) return 15 * 60;
  return 60 * 60;
}
