...
export class LogDatabase {
  ...
  private async createSchema(): Promise<void> {
    ...
    await db.query(
      `CREATE INDEX IF NOT EXISTS idx_ingestion_metrics_name_time
         ON ingestion_metrics (metric_name, time DESC)`,
    );
    await db.query(
      `CREATE INDEX IF NOT EXISTS idx_ingestion_metrics_project
    ...
    `);
    await db.query(
      `SELECT create_hypertable('worker_performance', 'time',
         chunk_time_interval => INTERVAL '1 hour', if_not_exists => TRUE)`,
    );
    await db.query(
      `CREATE INDEX IF NOT EXISTS idx_worker_perf_worker_time
    ...
    `);
    await db.query(
      `SELECT create_hypertable('admin_audit_log', 'time',
         chunk_time_interval => INTERVAL '1 day', if_not_exists => TRUE)`,
    );
    await db.query(
      `CREATE INDEX IF NOT EXISTS idx_admin_audit_category_time
    ...
    await db.query(
      `SELECT add_retention_policy('ingestion_metrics', INTERVAL '7 days', if_not_exists => TRUE)`,
    ).catch(() => {});
    await db.query(
      `SELECT add_retention_policy('worker_performance', INTERVAL '7 days', if_not_exists => TRUE)`,
    ).catch(() => {});
    await db.query(
      `SELECT add_retention_policy('admin_audit_log', INTERVAL '90 days', if_not_exists => TRUE)`,
    ).catch(() => {});
    await db.query(
      `SELECT add_retention_policy('ingestion_hourly_summary', INTERVAL '90 days', if_not_exists => TRUE)`,
    ).catch(() => {});
  }

  /** Insert one metric point. No-op when disabled. Never throws. */
  async writeMetric(name: string, value: number, opts: WriteMetricOptions = {}): Promise<void> {
    if (!this.enabled || !this.pool) return;
    try {
      await this.pool.query(
        `INSERT INTO ingestion_metrics (time, metric_name, value, project_id, org_id, tags, metadata)
  ...
  async writeWorkerPerformance(rec: WorkerPerformanceRecord): Promise<void> {
    if (!this.enabled || !this.pool) return;
    try {
      await this.pool.query(
  ...
  async writeAdminAudit(records: AdminAuditRecord[]): Promise<void> {
    if (!this.enabled || !this.pool || records.length === 0) return;
    const tuples: string[] = [];
  ...
  async queryMetrics(
    ...
    if (!this.enabled || !this.pool) return [];
    const r = await this.pool.query<MetricPoint>(
      `SELECT time, metric_name, value, project_id, tags
       FROM ingestion_metrics
