...
describe('ingestion quota enforcement', () => {
  it('throws QUOTA_EXCEEDED with structured details when blocked', async () => {
    const queue = { addBulk: vi.fn().mockResolvedValue(undefined), client: Promise.resolve({ ping: vi.fn() }) } as any;
    const cache = {
    ...
    const writer = { getProjectByApiKeyHash: vi.fn() } as any;
    const quotaService = {
    ...
    await expect(
      service.ingestBatch({ apiKey: 'test-api-key', events: [createRequestEvent()] })
