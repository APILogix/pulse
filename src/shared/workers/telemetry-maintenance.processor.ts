...
export class TelemetryMaintenanceWorker {
  ...
  constructor(
    ...
    private readonly opts: { intervalMs?: number; retentionMonths?: number } = {},
  ) {}
  ...
  private async dropExpiredPartitions(): Promise<void> {
    ...
    for (const table of PARTITIONED_TABLES) {
      ...
      const children = await this.pool.query<{ child: string }>(
        `SELECT inhrelid::regclass::text AS child
         FROM pg_inherits
         WHERE inhparent = $1::regclass`,
        [table],
      ).catch(() => ({ rows: [] as { child: string }[] }));

      for (const { child } of children.rows) {
        const match = /_y(\d{4})_m(\d{2})$/.exec(child);
        if (!match) continue; // skip DEFAULT and non-conventional partitions
  ...
  private addMonth(year: number, month: number): { y: number; m: number } {
